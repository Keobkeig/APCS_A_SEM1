    ...
    */
    ...
    public MazeWithExplorer(
        ...
        while( sc.hasNextLine() ) {
            ...
            for( int offsetEast = 0; offsetEast < line.length() ; offsetEast++) {
                String inChar = line.substring( offsetEast, offsetEast+1);
                int element;  // value destined for maze array
                
                // FOR TESTING CHANGE BACK LATER GRAHHHHHH 
    ...
    public String toString() {
        ...
        /* characters that represent MazeTerminals,
           ...
           and BREADCRUMB... 
           "." represents a breadcrumb with no explorer on it
           "@" represents a breadcrumb with an explorer on top of it 
    ...
    /**
      ...
      Replaces explorerIsOnA(), although that code might help here.
     */
    // statuses
    ...
    public int status() {
        int explorerStatus;  
        if ( (explorerOffsetSouth >= 0 && explorerOffsetSouth < maze.length) && 
             (explorerOffsetEast >= 0 && explorerOffsetEast< maze[0].length )){
            if ( maze[explorerOffsetSouth][explorerOffsetEast] == 0 ) 
                explorerStatus = STATUS_PATH_EXISTS;
            else if ( maze[explorerOffsetSouth][explorerOffsetEast] == 2 || 
                      maze[explorerOffsetSouth][explorerOffsetEast] == 3) 
                explorerStatus = STATUS_MAYBE_PATH_THRU_HERE;
            else  explorerStatus = STATUS_NO_PATH_THRU_HERE; 
